...
var getAccessToken = function(req, res, next) {
  ...
  var at = payload.at;
  console.log('Incoming access token: %s', at)
        ...
        if (!payload.m || payload.m == req.method) {
          if (!payload.u || payload.u == 'localhost:9002') {
            if (!payload.p || payload.p == req.path) {
              if (body.iss == 'http://localhost:9001/') {
                if ((Array.isArray(body.aud) && __.contains(body.aud, 'http://localhost:9002/')) || body.aud == 'http://localhost:9002/') {
