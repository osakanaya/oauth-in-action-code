	...
	if (req.body.grant_type == 'authorization_code') {
		...
		nosql.one(function(token) {
			...
			if (token) {
				 ...
				 */
				var rscope = getScopesFromForm(req.body);
        var cscope = token.scope;
        if (__.difference(rscope, cscope).length > 0) {
          res.status(400).json({error: 'invalid_scope'});
          return;
        }
        
        nosql.remove(function(found) { return (found.client_id == clientId);}, function(){});

				var access_token = randomstring.generate();
				nosql.insert({ access_token: access_token, client_id: clientId, scope: rscope.join(' ') });
        
        var refresh_token = randomstring.generate();
				nosql.insert({ refresh_token: refresh_token, client_id: clientId, scope: rscope.join(' ') });
